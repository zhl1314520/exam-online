...
import { getExamRecordsPage, getRecordAnswers, submitScores as submitScoresApi } from '@/api/examRecord'
...
const exportData = async () => {
  try {
    ...
    const response = await request.get('/records/export', {
      params,
      responseType: 'blob'
    })
...
const submitScores = async () => {
  try {
    const scores = subjectiveQuestions.value.map(item => ({
      answerId: item.answerId,
      teacherScore: item.teacherScore
    }))
    await submitScoresApi(currentRecord.value.recordId, scores)
