...
public class ClassController {
    ...
    @PostMapping("/add")
    public ResponseDTO<Class> add(@RequestBody Class clazz, HttpServletRequest request) {
        Class created = classService.add(clazz);
        systemLogService.log("新增", "新增班级：" + clazz.getClassName(), request);
        return ResponseDTO.success("添加成功", created);
    }

    @PutMapping("/update")
    public ResponseDTO<Class> update(@RequestBody Class clazz, HttpServletRequest request) {
        Class updated = classService.update(clazz);
        systemLogService.log("修改", "修改班级：" + clazz.getClassName(), request);
    ...
    public ResponseDTO<Void> delete(@PathVariable Integer id, HttpServletRequest request) {
        ...
        String detail = clazz != null ? "删除班级：" + clazz.getClassName() : "删除班级ID：" + id;
