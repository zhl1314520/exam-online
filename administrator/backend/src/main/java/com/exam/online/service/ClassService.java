...
public class ClassService {
    ...
    public Page<Class> getByPage(Integer pageNum, Integer pageSize, String keyword, Integer departmentId) {
        ...
        if (keyword != null && !keyword.isEmpty()) {
            classes = classes.stream()
                    .filter(c -> c.getClassName().contains(keyword) ||
                            c.getClassCode().contains(keyword) ||
                            (c.getDepartmentName() != null && c.getDepartmentName().contains(keyword)))
                    .toList();
        }
    ...
    public Class add(Class clazz) {
        Class existing = classMapper.selectAll().stream()
                .filter(c -> c.getClassCode().equals(clazz.getClassCode()))
                .findFirst()
                .orElse(null);
        if (existing != null) {
    ...
    public Class update(Class clazz) {
        ...
        Class sameCode = classMapper.selectAll().stream()
                .filter(c -> c.getClassCode().equals(clazz.getClassCode()) &&
                        !c.getClassId().equals(clazz.getClassId()))
                .findFirst()
                .orElse(null);
        if (sameCode != null) {
            throw new BusinessException(400, "班级代码已存在");
