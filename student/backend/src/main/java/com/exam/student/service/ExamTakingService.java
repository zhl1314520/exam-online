...
public class ExamTakingService {
    ...
    public Map<String, Object> submitExam(Integer recordId, Integer studentId) {
        ...
        // 更新考试记录
        examRecordMapper.updateSubmit(recordId);
        ...
        for (AnswerRecord answer : answers) {
            ...
            // 客观题自动评分
            if (question.getQuestionType() <= 3) { // 单选、多选、判断
                isCorrect = checkAnswer(question.getQuestionType(), answer.getStudentAnswer(), question.getCorrectAnswer());
                if (isCorrect) {
                    score = question.getScore();
                    objectiveScore += score;
                } else {
                    // 保存错题
                    saveWrongQuestion(studentId, question, answer.getStudentAnswer());
                }
            } else {
                // 主观题待教师批改
                subjectiveScore += 0;
