...
public class TeacherService {
    ...
    public Page<Teacher> getByPage(Integer pageNum, Integer pageSize, String keyword, Integer departmentId) {
        ...
        if (keyword != null && !keyword.isEmpty()) {
            teachers = teachers.stream()
                    .filter(t -> t.getTeacherNo().contains(keyword) ||
                            t.getRealName().contains(keyword) ||
                            (t.getDepartmentName() != null && t.getDepartmentName().contains(keyword)))
                    .toList();
        }
        if (departmentId != null) {
            teachers = teachers.stream()
                    .filter(t -> t.getDepartmentId().equals(departmentId))
                    .toList();
        }
    ...
    public Teacher add(Teacher teacher) {
        ...
        teacher.setPassword(PasswordUtil.md5(teacher.getPassword()));
        teacher.setStatus(1);
